...
void fourRingGoal(bool isRedTeam)
{
  ...
  else
  {
    ...
    intake.lift();
    intake.spin();
    chassis.moveToPoint(68, 28, 2500, {.minSpeed = 10}, false);
    intake.drop();
    chassis.moveLinear(-12, 1000, {.forwards = false, .minSpeed = 30});
    ...
    //* Corner
    chassis.turnToPoint(12, 0, 1000, {.minSpeed = 20}, false);
    pros::delay(200);
    arm.setIndex(0);
    intake.spin(-6000);
    chassis.moveToPoint(15, 17, 2000, {.maxSpeed = 80, .minSpeed = 40});
...
void debug(bool isRedTeam) { chassis.attemptReckonToGoal({48, 24, 0}, &clamp, 1000000); }
